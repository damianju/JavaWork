 ...
 * 3. 프로젝트 우클릭 - Configure Build Path - Library 탭 - Add Jars(혹은 라이브러리 추가) - 추가할 라이브러리 선택
 */
...
//		403  권한에러 Forbidden : 파일권한, 읽기권한, SSL, IP, 등...  <--- 웹 크롤링 하다가 은근히 자주 마주치게 되는 에러
		...
		System.out.println(doc.location()); //현재 웹문서의 url :https://www.naver.com/
		
		String outerHtml = doc.outerHtml(); // 현재 node의 outer html 텍스트
		//System.out.println(outerHtml);
		System.out.println(outerHtml.substring(0,200)+"...");
		
		// Document 나 Element 객체의 select(), selectFirst(), 메소드로 특정 element(들)을 추출
		System.out.println("[네이버 뉴스 캐스트]");
		
		element = doc.selectFirst("#news_cast"); // 검색된 Element들 중 최초 1개만 Element로 리턴 
		//System.out.println(element.outerHtml());
		Elements newsElements = doc.select("#news_cast li.ca_item"); // 검색된 Element들이 담겨있는 Elements 리턴
