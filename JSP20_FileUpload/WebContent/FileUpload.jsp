...
<%
	...
	// 서블릿 상의 upload 폳더의 물리적인 경로 얻어오기
	...
	try {  // jsp 파일에서 예외를 반드시 catch 할 필요는 없지만...
		...
		names = multi.getParameterNames(); // 일반 form parameter name 추출
		...
		while(names.hasMoreElements()){
			...
			out.println("파일 시스템 이름:" + fileSystemName + "<br>");
			...
			if(file != null){
				...
				if(bi != null) { // 이미지 파일 판정 여부
					...
					int heigt = bi.getHeight();
					out.println("이지미 파일 WxH:" + width + "x" + heigt);
					
				} else {
					out.println("이미지가 아립니다<br>");
				}
			}
			out.println("<hr>");
